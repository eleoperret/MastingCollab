...
parameters {
  ...
  real             mu_log_delta;
  vector[S]        log_delta_species;
  real<lower=0>    sigma_log_delta_species;
  vector[N_stands] log_delta_stand;
...
transformed parameters {
  ...
  // log_alpha_low  : grand_mean_low  + species NC + stand NC
  // log_alpha_high : log_alpha_low + log1p_exp(delta)   [enforces high > low]
  ...
  for (f in 1:F) {
    ...
    // log1p_exp mirrors the single-species parameterisation:
    //   high = low + log(1 + exp(delta))  — always > low
...
model {
  ...
  // --- Transitions ---
  grand_logit_theta1 ~ normal(1,   0.7);  // ~73 % stay in low state
  grand_logit_theta2 ~ normal(0,   0.7);  // ~50 % stay in mast state
  ...
  alpha_theta1_stand ~ normal(0, sigma_theta1_stand);
  alpha_theta2_stand ~ normal(0, sigma_theta2_stand);
  ...
  log_delta_species   ~ normal(0, sigma_log_delta_species);
  sigma_log_delta_species ~ normal(0, 1);
  log_delta_stand     ~ normal(0, sigma_log_delta_stand);
