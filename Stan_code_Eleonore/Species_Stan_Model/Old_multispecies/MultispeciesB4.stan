...
parameters {
  ...
  array[S] ordered[2] log_means;           // [s][1]=low mean, [s][2]=high mean
   ...
   // In parameters block — revert to non-centered for log_alpha
  vector[F] stand_effect_raw_low;    // ~ normal(0,1)
  vector[F] stand_effect_raw_high;   // ~ normal(0,1)
...
model {
  ...
  kappa_theta1 ~ gamma(6, 0.5);   // mean=12, away from zero
  kappa_theta2 ~ gamma(6, 0.5);
