...
parameters {
  ...
  real           mu_log_phi;
  vector[S]      phi_species;       // per-species overdispersion offset
  real<lower=0>  sigma_phi_species;
  vector[2]      phi_state;         // per-state overdispersion offset
}
...
for (s in 1:S)
  for (k in 1:2)
    log_phi[s, k] = mu_log_phi + phi_species[s] + phi_state[k];
...
model {
  ...
  grand_logit_theta2 ~ normal(0.2,   0.3); //Changed from the previous version MultispeciesNEWMike.stan 
