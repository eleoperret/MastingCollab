...
// I expect to have difference in species seed production during the low and high years, as for stand seed production (some are facing north, other south, some high altitude other low this could have some impact). 
// I expect to have transition probabilities that are different from species to species as they have different cycle and could potentially have different lows and highs at different time (more frequent etc.. based on literature)
//Overdispersion is the same everywhere. 

//Due to geometry issues : non-centering the high stand; and the thetas
// Same with the low state for both. Non-centering it also  (low stand and high/low species)

//I do not think that the model supports that there is a difference in the low state seed production between stands. I will then remove this. 
...
parameters {
  ...
  // I do not expect to have transition varing by stand
  ...
  // Stand random effects (only on th high years)
...
transformed parameters {
  ...
  for (f in 1:F) {
    ...
    int st = stand_id[f];
    log_alpha_low[f]  = grand_mean[1]  + alpha_low_species[s] ;
