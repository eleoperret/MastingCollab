...
parameters {
  ...
  real<lower=log_lambda> log_mu;// average seed production in high years years
...
transformed parameters {
  ...
  for (f in 1:F){
    ...
    for (t in start_id:end_id){
        log_omega[1,t] = poisson_log_lpmf(y[t] | log_lambda);//modfied some stuff here too
        log_omega[2,t] = neg_binomial_2_log_lpmf(y[t] | log_alpha[f], phi);//modfied some stuff here too
    }
  }
}
...
generated quantities {
  ...
  array [N] int<lower=1, upper= 2> state;
   ...
   for (f in 1:F){
    ...
    for (t in start_id:end_id){
      if(state[t] == 1){
        y_rep[t] = poisson_log_rng(log_lambda);
      }else{
        y_rep[t] = neg_binomial_2_log_rng(log_alpha[f], phi);
      }
