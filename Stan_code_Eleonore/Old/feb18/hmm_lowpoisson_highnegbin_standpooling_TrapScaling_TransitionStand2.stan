...
parameters {
  ...
  vector[F] stand_raw_unscaled;// stand deviations //Changes made here!!!
...
transformed parameters {
  ...
  vector[F] log_alpha;
  vector [F] stand_effect;
  //center the stand effects
  stand_effect = stand_raw_unscaled - mean (stand_raw_unscaled);
  //applying scaling
    log_alpha = log_mu + stand_effect * sigma;
...
model {
  ...
  stand_raw_unscaled ~ normal(0,1);//NEW
  phi ~ gamma(2, 0.1);
