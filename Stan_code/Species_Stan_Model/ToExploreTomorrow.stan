...
// Species-level baselines (key fix for state collapse)
// Single log1p_exp wrapping full delta predictor
// Stand effect shared across both states
// obs_missing handled correctly
...
data {
  ...
  array[F] int<lower=1, upper=S>        species_id;  // series-level
...
parameters {
  ...
  // ── Emission means ─────────────────────────────────────────────────────────
  // Species baseline: each species gets its own mu, pooled from grand hyperprior
  real             grand_mu_log_low;
  real<lower=0>    sigma_mu_log_low;
  vector[S]        mu_log_low_sp_nc;

  // Stand effect: additive, shared across species, shifts both states equally
  vector[N_stands] alpha_low_stand_nc;
  real<lower=0>    sigma_low_stand;

  // Delta: grand mean + species deviation, single log1p_exp
...
transformed parameters {
  // Species baselines (non-centered)
  ...
  // Delta species deviations (non-centered)
  ...
  for (f in 1:F) {
    ...
    // Stand effect shifts both states equally
    log_alpha_low_f[f]  = mu_log_low_sp[s] + alpha_low_stand[st];

    // Single log1p_exp wrapping full delta linear predictor
    log_alpha_high_f[f] = log_alpha_low_f[f]
                        + log1p_exp(grand_mu_log_delta + log_delta_sp[s]);
...
model {
  ...
  grand_mu_log_delta  ~ normal(1.5, 1.5);
  log_delta_sp_nc     ~ normal(0, 1);
  sigma_log_delta_sp  ~ normal(0, 1);
