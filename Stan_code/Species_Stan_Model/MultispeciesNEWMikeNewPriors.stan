...
//Overdispersion per state only (not per species)
...
model {
  ...
  // --- Transitions ---
  grand_logit_theta1 ~ normal(1.2,   0.7); //Changed from the previous version MultispeciesNEWMike.stan 
  grand_logit_theta2 ~ normal(0.5,   0.7); //Changed from the previous version MultispeciesNEWMike.stan 

  alpha_theta1_species_nc ~ normal(0, 1);
  alpha_theta2_species_nc ~ normal(0, 1);
  sigma_theta1_species    ~ normal(0, 0.7);//Changed from the previous version MultispeciesNEWMike.stan
  sigma_theta2_species    ~ normal(0, 0.7);//Changed from the previous version MultispeciesNEWMike.stan
  ...
  sigma_low_species    ~ normal(0.5, 1);//Changed from the previous version MultispeciesNEWMike.stan
  ...
  sigma_log_delta_species ~ normal(0, 1);//Changed from the previous version MultispeciesNEWMike.stan
  log_delta_stand_nc     ~ normal(0, 1);
  sigma_log_delta_stand  ~ normal(0, 1);//Changed from the previous version MultispeciesNEWMike.stan

  // Dispersion
  log_phi_state ~ normal(log(4), 0.6); //Changed from the previous version MultispeciesNEWMike.stan
