...
model {
  ...
  log_means[1][1] ~ normal(0, 0.8); // 0.3-3.7
  log_means[1][2] ~ normal(5.2, 0.6); // 55-403

  // CANO: q90=224 → log(224) ≈ 5.4
  log_means[2][1] ~ normal(2.3, 0.8);  // 1.2-16.4
  log_means[2][2] ~ normal(5.8, 0.8);   // 55-403

  // PSME: q90=123 → log(123) ≈ 4.8
  log_means[3][1] ~ normal(0, log(30)/2.57);// 0.1-8.2
  log_means[3][2] ~ normal(5.2, 0.5);   // 81-403

  // THPL: q90=707 → log(707) ≈ 6.6
  log_means[4][1] ~ normal(4,0.5);   // 4.1-54.6
  log_means[4][2] ~ normal(7,0.5);   // 1097-8103

  // TSHE: q90=1668 → log(1668) ≈ 7.4
  log_means[5][1] ~ normal(4,0.5);   // 20.1-148.4
  log_means[5][2] ~ normal(7,0.5) ;   // 2981-22026
