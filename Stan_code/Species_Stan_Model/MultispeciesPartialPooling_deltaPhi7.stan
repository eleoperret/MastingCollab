//HMM 2 state
//Transition matrix partially pooled by species and stand and non-centered
//Distribution: two NB for each state partially pooled by species and stand (with only stand state 2 nc) 
//Overdipserion per species and state
//Rho : per specie
//Forced state 2 above state 1 with delta-
//Last change: Grand_theta_1 : 0 instead of 1--> doesn't seem to improve
...
parameters {
  ...
  // Dispersion per species and state (NEW):
  matrix[S, 2] log_phi_species_state;  
  real mu_log_phi;
  real<lower=0> sigma_log_phi;
...
transformed parameters {
  ...
  for (f in 1:F) {
    ...
    for (t in start_id:end_id) {
      int s= sp[t];
      log_omega[1, t] = neg_binomial_2_log_lpmf(y[t] | log_alpha_low[f]  + log_area_ratio[t], exp(log_phi_species_state[s,1]));
      log_omega[2, t] = neg_binomial_2_log_lpmf(y[t] | log_alpha_high[f] + log_area_ratio[t], exp(log_phi_species_state[s,2]));
    }
  }
}
...
model {
  ...
  // Grand means
  grand_logit_theta1 ~ normal(0, 0.7); //changed those priors here too
  ...
  // Dispersion
  mu_log_phi ~ normal(log(4), 0.6);
  sigma_log_phi ~ normal(0, 0.5);
  //log_phi_species ~ normal(mu_log_phi, sigma_log_phi);
  
  for (s in 1:S)
    log_phi_species_state[s] ~ normal (mu_log_phi, sigma_log_phi);

  //For start state (rho)
  for (s in 1:S)
  rho [s] ~ dirichlet (rep_vector (2.0,2));

  for (f in 1:F) {
    int start_id = start_idxs[f];
    int end_id   = end_idxs[f];
    target += hmm_marginal(log_omega[, start_id:end_id], Gamma[f], rho[sp[start_idxs[f]]]);
  }
}
...
generated quantities {
  ...
  for (f in 1:F) {
    ...
    state[start_id:end_id] = hmm_latent_rng(
      log_omega[, start_id:end_id], Gamma[f], rho[sp[start_idxs[f]]]
    );

    for (t in start_id:end_id) {
      int s= sp[t];
      if (state[t] == 1)
        y_rep[t] = neg_binomial_2_log_rng(log_alpha_low[f]  + log_area_ratio[t], exp(log_phi_species_state[s, 1]));
      else
        y_rep[t] = neg_binomial_2_log_rng(log_alpha_high[f] + log_area_ratio[t], exp(log_phi_species_state[s,2]));
    }
  }
}
