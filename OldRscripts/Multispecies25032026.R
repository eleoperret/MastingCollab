...
seed_data<-seed_data %>%
  filter(spp %in% c("ABAM","CANO","PSME","TSHE","THPL", "ABLA"))
...
# Defining which species are valid at PARA and SUNR (adding this manually)
para_sunr_species <- tibble(
  stand = c("PARA", "PARA", "PARA", "SUNR", "SUNR", "SUNR"),
  spp   = c("ABLA", "ABAM", "CANO", "ABLA", "ABAM", "CANO")
)

# Combining
stands_long_extended <- bind_rows(stands_long, para_sunr_species)

# Filter
seed_filtered <- seed_data %>%
  semi_join(stands_long_extended, by = c("spp", "stand"))
...
stand_year_all <- seed_filtered %>%
  ...
  # Expanding each spp x stand to its own year sequence
  group_by(spp, stand) %>%
  tidyr::complete(year = min(year):max(year)) %>%
  mutate(
    # area: use median of observed years for missing years
    area = ifelse(is.na(area), 0.981, area) #based on the previous year and after
    # y stays NA for missing years — handled via is_obs in Stan
  ) %>%
  ungroup() %>%
  arrange(spp, stand, year)

# Quick check — SUNR should show one NA row in 2023
stand_year_all %>% 
  filter(stand == "SUNR") %>% 
  print(n = Inf)
...
stand_year_all <- stand_year_all %>%
  mutate(species_id = as.numeric(as.factor(spp)))

# Check species order
print(levels(as.factor(stand_year_all$spp)))  
# Should be: ABAM=1, ABLA=2, CANO=3, PSME=4, THPL=5, TSHE=6
...
# Check missing years are captured correctly
cat("Missing observations:", sum(is.na(stand_year_all$y)), "\n")
# Should be at least 1 (SUNR 2023) plus any others from 2009 expansion
...
stan_data_all <- list(
  ...
  end_idxs   = end_idxs,
  y          = ifelse(is.na(stand_year_all$y), 0L, as.integer(stand_year_all$y)),
  is_obs     = as.integer(!is.na(stand_year_all$y)),
...
fit_all <- stan(
  file    = "Stan_code/Species_Stan_Model/MultispeciesC3.stan",
  data    = stan_data_all,
...
pairs(fit_all, pars = c("sigma_phi", "mu_log_phi1", "sigma_theta", "logit_mu_theta1"))

pairs(fit_all, pars = c("sigma_phi", "mu_log_phi1[1]", "mu_log_phi2[1]", "phi1_raw[1]"))
pairs(fit_all, pars = c("logit_mu_theta1", "logit_mu_theta2", "sigma_theta", "theta_raw1","theta_raw2"))
